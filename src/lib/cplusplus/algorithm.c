...
static void TEST_for_each_fn(const int *el, void *arg0) {
	int * sum = arg0;
	*sum += *el;
}
static 	bool TEST_serach2_mypredicate (const int *i, const int *j, void *arg) {
	  return *i == *j;
	}

static char T_transform1_op_transform1(const int *el1, void *arg) {
	return *el1+'0';
}
static char T_transform2_op_transform2(const int *el1, const int *el2, void *arg) {
	return *el1+*el2+'0';
}
int algo_unittest()
{
