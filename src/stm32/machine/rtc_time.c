...
char *HW_RTC_DateTime_print_r(const struct HW_RTC_DateTime_s *dt, char result[])
{
...
static int HW_RTC_time_unittest_eq(volatile const time_t timev, volatile const struct HW_RTC_DateTime_s dt)
{
	const time_t timev2 = HW_RTC_DateTime_to_time_t(dt);
	struct HW_RTC_DateTime_s dt2 = HW_RTC_time_t_to_DateTime(timev);
...
{
#define TEST_eq(timeval, dtval, ...)  TEST(HW_RTC_time_unittest_eq(timeval, (struct HW_RTC_DateTime_s) dtval, ##__VA_ARGS__ ) == 0);
